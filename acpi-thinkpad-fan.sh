...
[ ${TEMP} -lt 40 ] && doas sysctl dev.acpi_ibm.0.fan_level=0 1> /dev/null 2> /dev/null && exit 0
[ ${TEMP} -lt 50 ] && doas sysctl dev.acpi_ibm.0.fan_level=1 1> /dev/null 2> /dev/null && exit 0
